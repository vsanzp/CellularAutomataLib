within CellularAutomataLib.Components.Basic;
model Animation "Model used to generate a graphical animation during the simulation"
  parameter Real Tstep = 1 "Duration time for each step/cycle";
  parameter Real initial_step = 0 "Time for starting the cycles";
  ...
  CellularAutomataLib.Components.Interfaces.CAport Space annotation (Placement(transformation(extent={{-10,100},{10,120}}),
        iconTransformation(extent={{-10,100},{10,120}})));
  Integer step( start= 0);
  CellularAutomataLib.Components.ExternalObj.Anim a=CellularAutomataLib.Components.ExternalObj.Anim(
      WindowWidth,
      WindowHeight,
      WindowX,
      WindowY,
  ...
  replaceable function SetDisplayFunction "Configures the display function used for the animation"
    extends
  ...
  function StepPlot "Execute an animation step"
