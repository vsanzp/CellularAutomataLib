...
model ExtInputRegionM "Values from the Matrix uM[] are used as inputs for TO.space CA (u[i,j,k] -> cell[i,j,k])"
  ...
  parameter Real interval = 1 "Quantum/threshold/interval value";
 ...
 // parameter Boolean start_below = false "start value for below";
  ...
  Real Y;
  Modelica.Blocks.Interfaces.RealVectorInput uM[XToRange[2]-XToRange[1]+1,YToRange[2]-YToRange[1]+1,ZToRange[2]-ZToRange[1]+1] annotation (
    ...
    for i in XToRange[1]:XToRange[2] loop
      for j in YToRange[1]:YToRange[2] loop
        for k in ZToRange[1]:ZToRange[2] loop
          ExtInput(TO.space, i, j, k, u[i,j,k], input_id);
        end for;
      end for;
    end for;
    preU := u;
...
equation */
  // TIME SAMPLER
algorithm
  when sample(start_sample,interval) then
    //Modelica.Utilities.Streams.print("SAMPLE: u "+String(u)+" id "+String(input_id));
    for i in XToRange[1]:XToRange[2] loop
      for j in YToRange[1]:YToRange[2] loop
        for k in ZToRange[1]:ZToRange[2] loop
          ExtInput(TO.space, i, j, k, uM[i,j,k], input_id);
          Y:=uM[i,j,k];
        end for;
      end for;
    end for;
